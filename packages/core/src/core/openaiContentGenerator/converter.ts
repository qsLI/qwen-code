...
export class OpenAIContentConverter {
  ...
  convertGeminiResponseToOpenAI(
    ...
    if (parsedParts.functionCalls.length > 0) {
      message.tool_calls = parsedParts.functionCalls.map(
        ({ call, signature }, index) => ({
    ...
    for (const part of parts) {
      if (typeof part === 'string') {
        ...
        functionCalls.push({
          ...
          signature: (part as any)?.extra_content?.google?.thought_signature,
        });
